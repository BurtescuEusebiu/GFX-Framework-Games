...
void Lab1::Update(float deltaTimeSeconds)
{
    ...
    // RenderMesh(meshes["sphere"], glm::vec3(0, 2, 0));

    if(jumping) {
        y_velocity += g * deltaTimeSeconds;
        pos.y += y_velocity * deltaTimeSeconds;
    ...
    RenderMesh(meshes[curr_obj], pos);
...
void Lab1::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F)
    {
        ...
        bg_color_changed = !bg_color_changed;
    }
    ...
    if (key == GLFW_KEY_SPACE && jumping == false)
    {
        original_y = pos.y;
        jumping = true;
        y_velocity = 3;
